...
function setDragging(activate) {
	if(arguments.length == 0 || activate) {
		$("#crystalBallLens").bind('mousedown' , function() {
			startDragging();
			$("#contentHolder").text("");
			$("#crystalBall").bind('mousemove', function() {
				shakeListener();
			});
...
function startDragging() {
	...
	var $ballStage = $('#crystalBallStage');
	var position = $ballStage.position();
	$(crystalInfo).trigger('stage-change', infoStageKey + '-process');
	var diameter = $("#crystalBallLens").width();
	var boundaries = [position.left, position.top, position.left + ($ballStage.width()-diameter), position.top + ($ballStage.height()-diameter)];
	$("#crystalBall").draggable({containment:boundaries});
	reverseEffect();
}
...
//Positions the crystall ball into the center of the container div
function moveToCenter(animate) {
	var $ballStage = $('#crystalBallStage');
	var $object = $("#crystalBall");
	var diameter = $("#crystalBallLens").width();
	var centerX = ($ballStage.width()-diameter) / 2;
	var centerY = ($ballStage.height()-diameter) / 2;
